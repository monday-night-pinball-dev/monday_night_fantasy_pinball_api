...
from tests.qdk.operators.sales_intake_jobs import SalesIntakeJobCreateModel, SalesIntakeJobModel, SalesIntakeJobSearchModel, create_sales_intake_job, get_sales_intake_job_by_id, get_sales_intake_jobs, sales_intake_job_hydration_check
...
def test_gets_sales_intake_job_by_id_with_hydration() -> None:
    populate_configuration_if_not_exists() 

    context: TestContext = TestContext(api_url = get_global_configuration().API_URL)

    posted_object = create_sales_intake_job(context, SalesIntakeJobCreateModel(create_parent_batch_job_if_null= True))

    result = get_sales_intake_job_by_id(context, posted_object.id, request_operators = RequestOperators(hydration_properties=["retailer", "retailer_location", "parent_batch_job"]))

    assert result is not None
    assert result.id == posted_object.id
    
    sales_intake_job_hydration_check(result)
...
def test_gets_sales_intake_jobs_with_ids_filter_with_hydration() -> None:
    populate_configuration_if_not_exists() 

    context: TestContext = TestContext(api_url = get_global_configuration().API_URL)

    posted_object_1: SalesIntakeJobModel = create_sales_intake_job(context, SalesIntakeJobCreateModel(create_parent_batch_job_if_null= True))
    posted_object_2: SalesIntakeJobModel = create_sales_intake_job(context, SalesIntakeJobCreateModel(create_parent_batch_job_if_null= True))
    posted_object_3: SalesIntakeJobModel = create_sales_intake_job(context, SalesIntakeJobCreateModel(create_parent_batch_job_if_null= True))
    posted_object_4: SalesIntakeJobModel = create_sales_intake_job(context, SalesIntakeJobCreateModel(create_parent_batch_job_if_null= True))

    filters: SalesIntakeJobSearchModel = SalesIntakeJobSearchModel(
        ids = f"{posted_object_1.id},{posted_object_2.id},{posted_object_3.id},{posted_object_4.id}"
    )
    
    result: PagedResponseItemList[SalesIntakeJobModel] = get_sales_intake_jobs(context, filters, request_operators = RequestOperators(hydration_properties=["retailer", "retailer_location", "parent_batch_job"]))
    ...
    posted_item_1: list[SalesIntakeJobModel] = [item for item in result.items if item.id == posted_object_1.id]
    assert len(posted_item_1) == 1  
    assert_objects_are_equal(posted_item_1[0], posted_object_1, ['parent_batch_job', 'retailer', 'retailer_location'])
    
    sales_intake_job_hydration_check(posted_item_1[0])

    posted_item_2: list[SalesIntakeJobModel] = [item for item in result.items if item.id == posted_object_2.id]
    assert len(posted_item_2) == 1 
    assert_objects_are_equal(posted_item_2[0], posted_object_2, ['parent_batch_job', 'retailer', 'retailer_location'])
    
    sales_intake_job_hydration_check(posted_item_2[0])
  
    posted_item_3: list[SalesIntakeJobModel] = [item for item in result.items if item.id == posted_object_3.id]
    assert len(posted_item_3) == 1 
    assert_objects_are_equal(posted_item_3[0], posted_object_3, ['parent_batch_job', 'retailer', 'retailer_location'])
    
    sales_intake_job_hydration_check(posted_item_3[0])
  
    posted_item_4: list[SalesIntakeJobModel] = [item for item in result.items if item.id == posted_object_4.id]
    assert len(posted_item_4) == 1 
    assert_objects_are_equal(posted_item_4[0], posted_object_4, ['parent_batch_job', 'retailer', 'retailer_location'])

    sales_intake_job_hydration_check(posted_item_4[0])
