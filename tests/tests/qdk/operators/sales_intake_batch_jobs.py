...
class SalesIntakeBatchJobModel():  
...
class SalesIntakeBatchJobSearchModel(PagingRequestModel):  
...
def mint_default_sales_intake_batch_job(
    context: TestContext, 
    overrides: SalesIntakeBatchJobCreateModel | None = None, 
    request_operators: RequestOperators | None = None
) -> SalesIntakeBatchJobCreateModel: 
    random_string = generate_random_string()

    overrides = overrides or SalesIntakeBatchJobCreateModel()
    
    default_sales_intake_batch_job: SalesIntakeBatchJobCreateModel = SalesIntakeBatchJobCreateModel(
    ...
    copy_object_when_appropriate(default_sales_intake_batch_job, overrides)
     
    return default_sales_intake_batch_job
...
def create_sales_intake_batch_job(
    ...
    post_object: SalesIntakeBatchJobCreateModel = mint_default_sales_intake_batch_job(context = context, overrides = overrides, request_operators = request_operators)

    result: Response = qa_post(context.api_url + "/sales_intake_batch_jobs", post_object, request_operators)
    ...
    return_object = SalesIntakeBatchJobModel(**result.json())
...
def get_sales_intake_batch_job_by_id(
    ...
    url: str = f"{context.api_url}/sales_intake_batch_jobs/{id}"
...
def get_sales_intake_batch_jobs(
        context: TestContext, 
        search_model: SalesIntakeBatchJobSearchModel | None,
        request_operators: RequestOperators | None = None 
    ) -> PagedResponseItemList[SalesIntakeBatchJobModel]: 

    url: str = f"{context.api_url}/sales_intake_batch_jobs"
    ...
    return_items: list[SalesIntakeBatchJobModel] = [SalesIntakeBatchJobModel(**obj) for obj in result_dict['items']]

    return_object = PagedResponseItemList[SalesIntakeBatchJobModel](
...
def update_sales_intake_batch_job(
        context: TestContext,
        id: str,
        update_model: SalesIntakeBatchJobUpdateModel | None = None, 
        request_operators: RequestOperators | None = None,
        allow_failures: bool = False
    ) -> SalesIntakeBatchJobModel:
    
    original_object: SalesIntakeBatchJobModel = get_sales_intake_batch_job_by_id(context, id, request_operators)

    result: Response = qa_patch(f"{context.api_url}/sales_intake_batch_jobs/{id}", update_model, request_operators)
