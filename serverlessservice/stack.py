...
class MnfpCoreServiceStack(cdk.Stack):
    def __init__(self, scope: cdk.App, construct_id: str, **kwargs) -> None:
        ...
        core_db_securitygroup = cdk.aws_ec2.SecurityGroup(
            self, f"{cds_prefix}-security-group", vpc=core_vpc, allow_all_outbound=True
        )
        ...
        service_credential_secret = cdk.aws_rds.DatabaseSecret(
            ...
            username="mnfp_service",
            dbname="mnfp",
            secret_name=f"{cds_prefix}-db-service-credentials",
        )

        database_credentials = cdk.aws_rds.Credentials.from_secret(
        ...
        database.grant_connect(lambda_function, db_user="mnfp_service")
