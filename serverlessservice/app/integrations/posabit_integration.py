...
from models.product_model import (
    ProductCreateModel,
    ProductModel,
    ProductVendorConfirmationStatuses,
)
from models.league_team_model import RetailerLocationModel


class PosabitSalesHistoryDiscountObject:
    def __init__(
        self, coupon_id: int | None = None, coupon_name: str | None = None
    ) -> None:
...
class PosabitSalesHistoryObject:
    def __init__(
        ...
        self.status = status
        self.items = (
            [PosabitSalesHistoryItemObject(**item) for item in items]
            if items is not None
            else []
        )
        self.sales_tenders = (
            [PosabitSalesHistorySalesTenderObject(**tender) for tender in sales_tenders]
            if sales_tenders is not None
            else []
        )
...
class PosabitIntegration:
    def __init__(self, service_caller: ServiceCaller = ServiceCaller()) -> None:
    ...
    def get_all_pages_of_inventory_items(
        ...
        running_list_of_inventory_items += [
            PosabitInventoryObject(**item) for item in posabit_inventories.inventory
        ]
        ...
        while current_page <= total_pages and simulator_response_id is None:
            ...
            next_page_of_posabit_inventories = PosabitInventoryResponse(
                **next_page_of_results.json()
            )
            running_list_of_inventory_items += [
                PosabitInventoryObject(**item) for item in posabit_inventories.inventory
            ]
    ...
    def get_all_pages_of_sales(
        ...
        running_list_of_sales_history_items += [
            PosabitSalesHistoryObject(**item)
            for item in posabit_sales_histories.sales_histories
        ]
        ...
        while current_page <= total_pages and simulator_response_id is None:
            ...
            next_page_of_posabit_inventories = PosabitSalesHistoriesResponse(
                **next_page_of_results.json()
            )
            running_list_of_sales_history_items += [
                PosabitSalesHistoryObject(**item)
    ...
    def get_inventory_snapshots(
        self, integration_key: str, simulator_response_id: UUID | None = None
    ) -> list[GenericInventoryObject] | None:
        return_list: list[GenericInventoryObject] = []

        inventory_items = self.get_all_pages_of_inventory_items(
            integration_key, simulator_response_id=simulator_response_id
        )

        for inventory_item in inventory_items:
            mnfp_inventory_snapshot_item = (
                self.convert_from_posabit_inventory_object_to_generic_inventory_object(
                    posabit_inventory_object=inventory_item
                )
            )

            return_list.append(mnfp_inventory_snapshot_item)
    ...
    def get_historical_sales(
        ...
        for sale in sales:
            mnfp_inventory_snapshot_item = self.convert_from_posabit_sales_history_object_to_generic_historical_sale_object(
    ...
    def convert_from_posabit_sales_history_object_to_generic_historical_sale_object(
        ...
        sales_object.items = [
            self.convert_from_posabit_sales_history_item_to_generic_historical_sale_item(
                item, posabit_sales_history_object
            )
            for item in posabit_sales_history_object.items
        ]
