...
def test_like_search_term_generates_sql_as_starts_with():
    search_term = LikeSearchTerm(
        "test_column", "A String", LikeComparatorModes.StartsWith
    )
...
def test_like_search_term_generates_sql_as_ends_with():
    search_term = LikeSearchTerm(
        "test_column", "A String", LikeComparatorModes.EndsWith
    )
...
def test_like_search_term_generates_sql_as_like_ignore_case():
    search_term = LikeSearchTerm(
        "test_column", "A String", LikeComparatorModes.Like, True
    )
...
def test_in_list_search_term_generates_sql():
    search_term = InListSearchTerm("test_column", ["string 1", "string 2", "string 3"])
...
def test_in_list_search_term_generates_sql_ignore_case():
    search_term = InListSearchTerm(
        "test_column", ["string 1", "string 2", "string 3"], True
    )

    expected_sqlstring = (
        f"LOWER(test_column) IN (\n"
        f"\tLOWER('string 1'),\n"
        f"\tLOWER('string 2'),\n"
        f"\tLOWER('string 3')\n"
        f")"
...
def test_range_search_term_generates_sql_no_min_max():
    ...
    expected_sqlstring: str = f"test_column = test_column OR test_column IS NULL"
...
def test_range_search_term_generates_sql_both_min_max_ignore_case():
    ...
    expected_sqlstring: str = (
        f"LOWER(test_column) >= LOWER('222') AND LOWER(test_column) <= LOWER('444')"
    )
