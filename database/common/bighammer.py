...
class BigHammer:
    def hammer(self, environment: Environment):
        ...
        print(
            f"Retrieved {len(all_tables)} tables: \n[\n\t{([t.table_name for t in all_tables])}\n]"
        )
        ...
        for table in all_tables:
            ...
            all_foreign_keys = (
                environment.pg_connection.select_all_foreign_keys_for_table(
                    table.table_name
                )
            )

            print(
                f"Retrieved {len(all_foreign_keys)} foreign keys on table {table.table_name}: \n[\n\t{([t.fk_name for t in all_foreign_keys])}]\n"
            )

            for fk in all_foreign_keys:
                print(f"Deleting Foreign Key {fk.fk_name}...")

                environment.pg_connection.delete_foreign_key(
